...
describe('encrypted history transfer', () => {
  it('round-trips complete history', async () => {
    ...
    await expect(decryptHistory(await encryptHistory(entries, key), key)).resolves.toEqual(entries)
  })

  it('uses a fresh nonce for every export', async () => {
    const entries = upsertHistory([], { site: 'example.com', counter: 1, template: 'long' })
  ...
  it('rejects another identity and modified data', async () => {
    ...
    await expect(decryptHistory(encoded, otherKey)).rejects.toThrow('身份不匹配')
    const replacement = encoded.endsWith('A') ? 'B' : 'A'
    await expect(decryptHistory(`${encoded.slice(0, -1)}${replacement}`, key)).rejects.toThrow('身份不匹配')
  })
})