...
async function importAesKey(
  ...
  return crypto.subtle.importKey('raw', asBufferSource(keyBytes), 'AES-GCM', false, [usage])
}
...
export async function decryptHistory(
  ...
  try {
    if (encoded.length > MAX_ENCODED_BYTES * 2) throw new Error('Oversized data')
    ...
    if (serialized.length > MAX_ENCODED_BYTES) throw new Error('Oversized data')
    ...
    if (entries.some((entry) => entry === null)) throw new Error('Invalid entry')
