...
describe("App session workflow", () => {
  beforeEach(() => {
    cleanup();
    localStorage.clear();
    localStorage.setItem("mpw-web-language", "zh-CN");
  ...
  it("switches the interface language and persists the selection", () => {
    const languageButton = screen.getByRole("button", {
      name: "当前为中文，切换为English",
    });

    fireEvent.click(languageButton);

    expect(
      screen.getByRole("heading", { name: "Unlock offline key" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("textbox", { name: "Full name" }),
    ).toBeInTheDocument();
    expect(localStorage.getItem("mpw-web-language")).toBe("en");
  });
  ...
  it("imports encrypted history from text", async () => {
    ...
    fireEvent.change(
      screen.getByRole("textbox", { name: "要导入的迁移文本" }),
      {
  ...
  it("filters website history from the mobile search box", async () => {
    ...
    expect(
      within(mobileHistory!).getByRole("button", {
        name: /载入 alpha\.example/,
      }),
    ).toBeInTheDocument();
    expect(
      within(mobileHistory!).queryByRole("button", {
        name: /载入 beta\.example/,
      }),
    ).not.toBeInTheDocument();

    fireEvent.change(mobileSearch, { target: { value: "missing" } });
    expect(
      within(mobileHistory!).getByText("没有匹配的网站"),
    ).toBeInTheDocument();
  });
