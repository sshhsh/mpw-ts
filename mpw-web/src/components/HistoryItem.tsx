import { X } from "lucide-react";

import type { SiteHistoryEntry } from "../lib/history";
import { templateName } from "../lib/templateMetadata";
import { useLanguage } from "../lib/useLanguage";

interface HistoryItemProps {
  entry: SiteHistoryEntry;
  selected: boolean;
  variant: "desktop" | "mobile";
  managing?: boolean;
  onLoad: (entry: SiteHistoryEntry) => void;
  onRemove: (id: string) => void;
}

function formatRelativeTime(
  timestamp: number,
  t: ReturnType<typeof useLanguage>["t"],
): string {
  const minutes = Math.floor(Math.max(0, Date.now() - timestamp) / 60_000);
  if (minutes < 1) return t("history.time.justNow");
  if (minutes < 60) return t("history.time.minutes", { count: minutes });
  const hours = Math.floor(minutes / 60);
  if (hours < 24) return t("history.time.hours", { count: hours });
  return t("history.time.days", { count: Math.floor(hours / 24) });
}

function HistoryItem({
  entry,
  selected,
  variant,
  managing = false,
  onLoad,
  onRemove,
}: HistoryItemProps) {
  const { t } = useLanguage();
  const localizedTemplateName = templateName(entry.template, t);
  const relativeTime =
    variant === "desktop" ? formatRelativeTime(entry.lastUsedAt, t) : "";
  const content = (
    <>
      <span className="monogram">{entry.site.charAt(0).toUpperCase()}</span>
      <span>
        <strong>{entry.site}</strong>
        <small>
          {localizedTemplateName} ·{" "}
          {t("history.counter", { counter: entry.counter })}
          {relativeTime ? ` · ${relativeTime}` : ""}
        </small>
      </span>
    </>
  );
  const loadLabel = t("history.load", {
    site: entry.site,
    template: localizedTemplateName,
    counter: entry.counter,
  });
  const removeButton = (
    <button
      className={variant === "desktop" ? "delete-button" : "shortcut-delete"}
      type="button"
      onClick={() => onRemove(entry.id)}
      aria-label={t("history.delete", { site: entry.site })}
    >
      <X size={variant === "desktop" ? 15 : 14} />
    </button>
  );

  if (variant === "desktop") {
    return (
      <article className={`history-item ${selected ? "selected" : ""}`}>
        <button
          className="history-load"
          type="button"
          onClick={() => onLoad(entry)}
          aria-label={loadLabel}
        >
          {content}
        </button>
        {removeButton}
      </article>
    );
  }

  return (
    <div className={`shortcut-wrap ${selected ? "selected" : ""}`}>
      <button
        className="history-shortcut"
        type="button"
        onClick={() => onLoad(entry)}
        aria-label={loadLabel}
      >
        {content}
      </button>
      {managing && removeButton}
    </div>
  );
}

export default HistoryItem;
